...
async function main() {
  ...
  for (const chunk of chunks) {
    const label = chunk.hero ? `HERO  ${chunk.hero} (${chunk.role})` : `SECTION  ${chunk.section}`;
    const firstLine = chunk.text.split("\n")[0].slice(0, 80);
    console.log(`${label.padEnd(32)} ${chunk.text.length.toString().padStart(5)} chars | ${firstLine}`);
  }
}
